...
async function fetchNvidiaNimModelIds(): Promise<Map<string, string>> {
	try {
		const localCachePath = join(__dirname, "nvidia-models.json");
		let data: { data?: NvidiaNimModelListItem[] };
		if (existsSync(localCachePath)) {
			console.log("Using local NVIDIA NIM cache (scripts/nvidia-models.json)");
			data = JSON.parse(readFileSync(localCachePath, "utf8")) as { data?: NvidiaNimModelListItem[] };
		} else {
			...
			data = (await response.json()) as { data?: NvidiaNimModelListItem[] };
		}
...
async function fetchOpenRouterModels(): Promise<Model<any>[]> {
	try {
		const localCachePath = join(__dirname, "openrouter-models.json");
		let data: { data?: OpenRouterModelListItem[] };
		if (existsSync(localCachePath)) {
			console.log("Using local OpenRouter cache (scripts/openrouter-models.json)");
			data = JSON.parse(readFileSync(localCachePath, "utf8")) as { data?: OpenRouterModelListItem[] };
		} else {
			console.log("Fetching models from OpenRouter API...");
			const response = await fetch("https://openrouter.ai/api/v1/models");
			if (!response.ok) throw new Error(`OpenRouter API returned ${response.status}`);
			data = (await response.json()) as { data?: OpenRouterModelListItem[] };
		}
...
async function fetchAiGatewayModels(): Promise<Model<any>[]> {
	try {
		const localCachePath = join(__dirname, "ai-gateway-models.json");
		let data: { data?: AiGatewayModel[] };
		if (existsSync(localCachePath)) {
			console.log("Using local Vercel AI Gateway cache (scripts/ai-gateway-models.json)");
			data = JSON.parse(readFileSync(localCachePath, "utf8")) as { data?: AiGatewayModel[] };
		} else {
			console.log("Fetching models from Vercel AI Gateway API...");
			const response = await fetch(`${AI_GATEWAY_MODELS_URL}/models`);
			if (!response.ok) throw new Error(`Vercel AI Gateway API returned ${response.status}`);
			data = (await response.json()) as { data?: AiGatewayModel[] };
		}
		const models: Model<any>[] = [];
...
async function loadModelsDevData(): Promise<Model<any>[]> {
	try {
		const localCachePath = join(__dirname, "models-dev-api.json");
		let data: ModelsDevCatalog;
		if (existsSync(localCachePath)) {
			console.log("Using local models.dev cache (scripts/models-dev-api.json)");
			data = JSON.parse(readFileSync(localCachePath, "utf8")) as ModelsDevCatalog;
		} else {
